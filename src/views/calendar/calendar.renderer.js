...
export function renderMonthGrid(currentDate, tasks) {
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    const dayTasks = tasksMap.get(cellDateStr) || [];
        ...
        <div class="flex sm:hidden flex-wrap items-center justify-center gap-0.5 mt-auto pt-1">
          ${dayTasks
            ...
            .map((task) => {
              ...
              const dotBg = priorityDots[task.priority] || priorityDots.low;
              return `<span class="w-1.5 h-1.5 rounded-full ${dotBg}"></span>`;
            })
            .join("")}
          ${dayTasks.length > 3 ? `<span class="w-1 h-1 rounded-full bg-tertiary"></span>` : ""}
        </div>
      </div>
    `;
  }
...
export function renderDayList(currentDate, tasks) {
...
export function renderYearHeatmap(currentDate, tasks) {
  ...
  tasks.forEach((t) => {
    if (!t.dueDate) return;
    tasksMap.set(t.dueDate, (tasksMap.get(t.dueDate) || 0) + 1);
  });
  ...
  const renderMonthDots = (year, monthIndex, isOverflow = false) => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const count = isOverflow ? 0 : tasksMap.get(dStr) || 0;
