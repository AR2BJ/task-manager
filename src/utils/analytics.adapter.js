 ...
 * using task creation dates and completed subtasks/statuses.
 */
function getActivityMap(tasks) {
  const map = {};

  tasks.forEach((task) => {
    // Increment activity for creation date
    if (task.createdAt) {
      const createdIso = task.createdAt.split("T")[0];
...
export const AnalyticsAdapter = {
  generateHeatmapSeries(tasks = [], view = "weekly") {
    ...
    const globalActivityMap = getActivityMap(tasks);
